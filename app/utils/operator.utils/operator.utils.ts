...
export class OperatorUtils {
  ...
  static getRectListInYRange(yRange: RangeInfo, rectList: RectInfo[]): RectInfo[] {
  ...
  static getRectListInXRange(xRange: RangeInfo, rectList: RectInfo[]): RectInfo[] {
    return rectList.reduce<RectInfo[]>((previous, current) => {
  ...
  static getRectDistance(rect: RectInfo, parentRect: RectInfo): number {
    if (rect.x > (parentRect.x + parentRect.width)) {
      if (rect.y >= parentRect.y && rect.y <= parentRect.y + parentRect.height) {
        return rect.x - (parentRect.x + parentRect.width);
      }
      if (rect.y < parentRect.y) {
        return Math.sqrt((parentRect.y - (rect.y + rect.height)) ** 2 + (rect.x - (parentRect.x + parentRect.width)) ** 2);
      }
      return Math.sqrt((rect.y - (parentRect.y + parentRect.height)) ** 2 + (rect.x - (parentRect.x + parentRect.width)) ** 2);
    }
    if (parentRect.x > (rect.x + rect.width)) {
      ...
      if (rect.y < parentRect.y) {
        return Math.sqrt((parentRect.y - (rect.y + rect.height)) ** 2 + (parentRect.x - (rect.x + rect.width)) ** 2);
      }
      return Math.sqrt((rect.y - (parentRect.y + parentRect.height)) ** 2 + (parentRect.x - (rect.x + rect.width)) ** 2);
    }
