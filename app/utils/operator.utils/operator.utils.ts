...
export class OperatorUtils {
  ...
  static getRectListInYRange(yRange: RegionInfo, rectList: RectInfo[]): RectInfo[] {
  ...
  static getRectListInXRange(xRange: RegionInfo, rectList: RectInfo[]): RectInfo[] {
    return rectList.reduce<RectInfo[]>((previous, current) => {
  ...
  static getRectDistance(rect: RectInfo, parentRect: RectInfo): number {
    ...
    if (rect.x > parentRect.x + parentRect.width) {
      if (
        (rect.y + rect.height > parentRect.y && rect.y < parentRect.y + parentRect.height) ||
    ...
    } else if (rect.x + rect.width < parentRect.x) {
      if (
        (rect.y + rect.height > parentRect.y && rect.y < parentRect.y + parentRect.height) ||
        Math.abs(rect.y + rect.height - parentRect.y) <
          Math.abs(parentRect.x - rect.x - rect.width) ||
        Math.abs(rect.y - parentRect.y - parentRect.height) <
          Math.abs(parentRect.x - rect.x - rect.width)
      ) {
