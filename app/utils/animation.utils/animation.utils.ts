...
import { AnimationGroupInfoList, AnimationKeyFrameTransform, AnimationKeyFrames } from '../../type/animation.type';
...
export class AnimationUtils {
	...
	static getAnimationGroupKeyframes(animation: DslAnimationType): AnimationKeyFrames | undefined {
		...
		const animationInfo = AnimationGroupInfoList.find(
			info => info.effect === (animation.content as AnimationGroupType).effect
		);
		if (animationInfo) {
			return animationInfo.achieveAnimationKeyFrames(
				(animation.content as AnimationGroupType).direction
			);
		}
		return undefined;
	}
