...
export class DslDiffHelper {
  static getDslDiffInfo(params: {
    ...
    const diff = DslDiffHelper.getDslDiffMapping(params.originDsl, params.newDsl);
    const result: {
      id: string;
      path: string;
      oldValue?: string | number | boolean | { [key: string]: any };
    ...
    Object.keys(diff).forEach(path => {
      result.push({
        id: params.id,
        path,
        oldValue: DslDiffHelper.getDslValueByPath(params.originDsl, path),
        newValue: DslDiffHelper.getDslValueByPath(params.newDsl, path)
      });
    });
    if (params.rootPath) {
      result.forEach(item => {
  ...
  static getDslDiffMapping(originDsl: { [key: string]: any }, newDsl: { [key: string]: any }) {
    const originMapping = DslDiffHelper.flattenJSON(originDsl);
    const updateMapping = DslDiffHelper.flattenJSON(newDsl);
