...
const easingFunction: any = {
  easeInLinear(x: number) {
    return x;
  },
  easeInQuad(x: number) {
    return x * x;
  ...
  easeInCircle(x: number) {
    return 1 - sqrt(1 - pow(x, 2));
  },
  easeOutCircle(x: number) {
    return sqrt(1 - pow(x - 1, 2));
  },
  easeInOutCircle(x: number) {
...
const easing = Object.values(easingFunction);
